...
internal fun newsReceiver(onNewsList: ((List<News>) -> Unit)) {
    ...
    onNewsList.invoke(newsList)
}
internal fun newsTitleReceiver(onTitleList: ((List<TitleBean>) -> Unit)) {
    val titleList = createTitleList()
    onTitleList.invoke(titleList)
}
//顶层声明中的方法全局可用，可以直接调用
...
fun createTitleList() = mutableListOf<TitleBean>().apply {
    for (i in 1..10) {
